    ...
    : sdaPin(sdaPin), sclPin(sclPin), i2cInst(i2cInst)
{
    ...
    if (reset == 1)
    {
        ...
        // writeByte(SYSTEM__MODE_GPIO1, 0x10); // Enables polling for ‘New Sample ready’
        // when measurement completes
        writeByte(READOUT__AVERAGING_SAMPLE_PERIOD, 0x30); // Set the averaging sample period
        // (compromise between lower noise and
        // increased execution time)
        writeByte(SYSALS__ANALOGUE_GAIN, 0x46); // Sets the light and dark gain (upper
        // nibble). Dark gain should not be
        // changed.
        writeByte(SYSRANGE__VHV_REPEAT_RATE, 0xFF); // sets the # of range measurements after
        // which auto calibration of system is performed
        writeByte(SYSALS__INTEGRATION_PERIOD, 0x63); // Set ALS integration time to 100ms

        writeByte(SYSRANGE__VHV_RECALIBRATE, 0x01); // perform a single temperature calibration
        // of the ranging sensor
        // Optional: Public registers - See data sheet for more detail
        writeByte(SYSRANGE__INTERMEASUREMENT_PERIOD, 0x09); // Set default ranging inter-measurement
        // period to 100ms
        writeByte(SYSALS__INTERMEASUREMENT_PERIOD, 0x31); // Set default ALS inter-measurement period
        // to 500ms
        writeByte(SYSTEM__INTERRUPT_CONFIG_GPIO, 0x24); // Configures interrupt on ‘New Sample
        // Ready threshold event’

        // Reset other settings to power-on defaults

        // sysrange__max_convergence_time = 49 (49 ms)
        writeByte(SYSRANGE__MAX_CONVERGENCE_TIME, 0x31);

        // disable interleaved mode
        writeByte(INTERLEAVED_MODE__ENABLE, 0);
