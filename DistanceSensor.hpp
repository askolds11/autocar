...
private:
    ...
    /// @brief Split 16-bit register address into two bytes and write the address + data via I²C
    /// @param reg
    /// @param data
    void writeByte(uint16_t reg, uint8_t data);

    /// @brief Split 16-bit register address into two bytes and write required register address to VL6180 and read the data back
    /// @param reg
    /// @return
...
public:
    ...
    /// @param sclPin
    /// @param i2cInst
    DistanceSensor(int sdaPin, int sclPin, i2c_inst *i2cInst);
